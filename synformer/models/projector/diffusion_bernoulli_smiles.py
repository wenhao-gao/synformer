...
class BernoulliDiffusionSMILESProjector(BaseProjector):
    def __init__(self, cfg: ProjectorConfig | None = None) -> None:
        super().__init__()
        cfg = cfg or ProjectorConfig()
        self.cfg = cfg

        self.smiles_emb = nn.Embedding(cfg.enc.num_token_types, cfg.enc.d_model, padding_idx=0)
        self.pe_enc = PositionalEncoding(
            d_model=cfg.enc.d_model,
            max_len=cfg.enc.pe_max_len,
        )
        self.enc = nn.TransformerEncoder(
            ...
            num_layers=cfg.enc.num_layers,
            norm=nn.LayerNorm(cfg.enc.d_model),
        ...
        self.pe_dec = PositionalEncoding(
