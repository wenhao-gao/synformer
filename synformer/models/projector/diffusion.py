...
from synformer.models.diffusion import EuclideanDiffusion, NoiseConditionedMLP
...
class DiffusionProjector(BaseProjector):
    def __init__(self, cfg: ProjectorConfig | None = None) -> None:
        ...
        self.fingerprint_var_sched = EuclideanDiffusion(num_steps=cfg.dec.diffusion_num_steps, s=cfg.dec.diffusion_s)
        self.fingerprint_denoiser = NoiseConditionedMLP(
            dim_in=cfg.dec.d_model + cfg.dec.fingerprint_dim * 3,
    ...
    @staticmethod
    def normalize_fingerprint(fingerprint: torch.Tensor) -> torch.Tensor:
        """
        Scale the fingerprint from [0, 1] to [-0.5, 0.5]
        """
        return fingerprint - 0.5

    @staticmethod
    def denormalize_fingerprint(fingerprint: torch.Tensor) -> torch.Tensor:
        """
        Scale the fingerprint from [-0.5, 0.5] to [0, 1]
        """
        return fingerprint + 0.5
    ...
    def predict_noise(
        ...
        x_t: torch.Tensor,
        t: torch.Tensor,
        var_sched: EuclideanDiffusion | None = None,
    ) -> torch.Tensor:
        return self.fingerprint_denoiser(
            torch.cat([h, x_t, torch.sin(x_t), torch.cos(x_t)], dim=-1),
            t=t,
    ...
    def get_loss(
        ...
        **options,
        ...
        fp_gt_norm = self.normalize_fingerprint(reactant_fps_out)  # (bsz, len, fp_dim)
        timestep = torch.randint(
            low=1,
            high=self.fingerprint_var_sched.num_steps,
            size=[num_timestep_samples, *fp_gt_norm.shape[:-1]],
            device=fp_gt_norm.device,
        )
        fp_noisy, noise_gt = self.fingerprint_var_sched.add_noise(
            x_0=fp_gt_norm[None].repeat(num_timestep_samples, 1, 1, 1),
            t=timestep,
        )
        noise_pred = self.predict_noise(h=y[None].repeat(num_timestep_samples, 1, 1, 1), x_t=fp_noisy, t=timestep)
        loss_fingerprint = (
            F.mse_loss(noise_pred, noise_gt, reduction="none").mean(0).sum(-1) * fingerprint_flag
        ).sum() / (fingerprint_flag.sum() + 1e-6)
        y_fingerprint = self.denormalize_fingerprint(
            self.fingerprint_var_sched.denoise(fp_noisy, noise_pred, timestep)
        )
    ...
    def generate_fingerprint(
        ...
        var_sched = var_sched or self.fingerprint_var_sched
        if x_0 is None:
            x_0 = torch.randn([*h.shape[:-1], self.fingerprint_dim], dtype=h.dtype, device=h.device)

        x_prev = x_0
        for t_index in range(var_sched.num_steps, 0, -1):
            t = torch.full(h.shape[:-1], fill_value=t_index, dtype=torch.long, device=h.device)
            noise_pred = self.predict_noise(h=h, x_t=x_prev, t=t, var_sched=var_sched)
            x_prev = var_sched.denoise(x_t=x_prev, eps=noise_pred, t=t)
        return self.denormalize_fingerprint(x_prev)
    ...
    def predict(
        ...
        *,
        fingerprint_var_sched: EuclideanDiffusion | None = None,
        ...
        **options,
        ...
        fp_query = y_fingerprint.clamp(min=0.0, max=1.0).detach()  # (bsz, n_fps, fp_dim)
